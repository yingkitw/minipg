...
use crate::traits::GrammarParser as GrammarParserTrait;
...
impl GrammarComposer {
    ...
    fn resolve_import(&mut self, grammar: &mut Grammar, import_name: &str) -> Result<()> {
        ...
        // Cache it
        self.grammar_cache
            .insert(import_name.to_string(), imported_grammar.clone());
    ...
    fn find_grammar_file(&self, import_name: &str) -> Result<PathBuf> {
        ...
        Err(Error::parse(
            ...
            format!(
                "grammar file not found: {} (searched in {:?})",
    ...
    pub fn merge_grammar(&self, target: &mut Grammar, source: &Grammar) -> Result<()> {
        ...
        let target_rule_names: HashSet<String> =
            target.rules.iter().map(|r| r.name.clone()).collect();
        let source_rule_names: HashSet<String> =
            source.rules.iter().map(|r| r.name.clone()).collect();
...
mod tests {
    ...
    fn test_merge_grammar_rules() {
        let mut target = Grammar::new(
            "Target".to_string(),
            crate::types::GrammarType::Combined,
        );
        let mut source = Grammar::new(
            "Source".to_string(),
            crate::types::GrammarType::Combined,
        );
        ...
        let composer = GrammarComposer::new();
        composer
            .merge_grammar(&mut target, &source)
            .expect("merge failed");
    ...
    fn test_merge_grammar_options() {
        ...
        assert_eq!(
            target.options.get("key1").map(|s| s.as_str()),
            Some("value1")
        );
        assert_eq!(
            target.options.get("key2").map(|s| s.as_str()),
            Some("value2")
        );
    }
