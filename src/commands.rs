...
use minipg::codegen::CodeGenerator;
use minipg::core::{
...
fn generate_command(
    ...
    for diagnostic in &analysis.diagnostics {
        match diagnostic.severity {
            minipg::DiagnosticSeverity::Error => error!("{}", diagnostic),
            minipg::DiagnosticSeverity::Warning => {
                tracing::warn!("{}", diagnostic)
            }
            minipg::DiagnosticSeverity::Info => info!("{}", diagnostic),
        }
    }
...
fn validate_command(input: impl AsRef<Path>) -> Result<()> {
    ...
    for diagnostic in &analysis.diagnostics {
        match diagnostic.severity {
            minipg::DiagnosticSeverity::Error => {
                error!("{}", diagnostic);
                has_errors = true;
            }
            minipg::DiagnosticSeverity::Warning => {
