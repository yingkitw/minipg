...
use crate::ast::{Grammar, Rule, Element};
use crate::incremental::{Range, Position, Point};
...
impl QueryMatcher {
    ...
    fn match_rule(&self, pattern: &Pattern, rule: &Rule, source: &str, pattern_idx: usize) -> Option<Vec<Match>> {
        let mut matches = Vec::new();
    ...
    fn match_element(&self, pattern: &Pattern, element: &Element, _source: &str) -> Option<Vec<Capture>> {
        let mut captures = Vec::new();
...
mod tests {
    ...
    fn test_match_rule_name() {
        let pattern = Pattern::new(
            PatternNode::new("test_rule".to_string())
                .with_capture("rule".to_string())
        );
