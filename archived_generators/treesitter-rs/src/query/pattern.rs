...
mod tests {
    ...
    fn test_pattern_node_with_capture() {
        let node = PatternNode::new("identifier".to_string()).with_capture("variable".to_string());
        assert_eq!(node.capture, Some("variable".to_string()));
    }

    #[test]
    fn test_pattern_node_with_field() {
        let node = PatternNode::new("identifier".to_string()).with_field("name".to_string());
    ...
    fn test_pattern_extract_captures() {
        let root = PatternNode::new("function".to_string()).with_children(vec![
            PatternNode::new("identifier".to_string())
                .with_field("name".to_string())
                .with_capture("function.name".to_string()),
            PatternNode::new("parameters".to_string()).with_capture("function.params".to_string()),
        ]);
