    ...
    fn generate_source(&self, grammar: &Grammar) -> String {
        ...
        if !lexer_rules.is_empty() {
            ...
            for (i, rule) in lexer_rules.iter().enumerate() {
                ...
                code.push_str(") {\n");
                code.push_str(
                    "        std::string text = input.substr(start_pos, position - start_pos);\n",
                );
                code.push_str(&format!(
                    "        return Token(TokenType::{}, text, line, column, text.length());\n",
                    rule.name
                ));
            }
            code.push_str("    }\n\n");
            ...
            for rule in lexer_rules {
                ...
                if let Some(alt) = rule.alternatives.first() {
                    for element in &alt.elements {
                        code.push_str(&super::pattern_match::generate_element_match_cpp(
                            element, "    ",
                        ));
                    }
                }
