...
use minipg_core::{traits::GrammarParser as GrammarParserTrait, Result};
...
impl SyntaxTree {
    pub fn new(grammar: Grammar, source: String) -> Self {
        ...
        Self {
            grammar,
            source,
            range: Range::new(Position::zero(), Position::new(end_byte, end_point)),
            subtrees: Vec::new(),
        }
    }
...
pub struct DefaultIncrementalParser {
    parser: minipg_antlr::parser::GrammarParser,
...
impl DefaultIncrementalParser {
    pub fn new() -> Self {
        Self {
            parser: minipg_antlr::parser::GrammarParser::new(),
    ...
    fn fast_incremental_parse(
        ...
        let metrics = ParseMetrics::new(duration).with_incremental(true, 1, 0); // Reused the old tree effectively
...
mod tests {
    ...
    fn test_syntax_tree_creation() {
        let grammar = Grammar::new(
            "Test".to_string(),
            minipg_core::types::GrammarType::Combined,
        );
    ...
    fn test_parse_metrics() {
        ...
        let metrics = ParseMetrics::new(Duration::from_millis(10)).with_incremental(true, 5, 2);
        assert!(!metrics.meets_target());
        assert!(metrics.incremental_hit);
    ...
    fn test_incremental_performance() {
        ...
        let edit = Edit::insert(source.len(), Point::new(2, 9), "");
    ...
    fn test_multiple_edits_sequence() {
        ...
        let (tree2, metrics1) = parser
            .parse_incremental_with_metrics(&tree1, &edit1)
            .unwrap();
        assert!(metrics1.duration.as_millis() >= 0);

        // Second edit on the updated tree
        let edit2 = Edit::insert(source.len() + 11, Point::new(2, 9), "\nID2: [0-9]+;");
        let (tree3, metrics2) = parser
            .parse_incremental_with_metrics(&tree2, &edit2)
            .unwrap();
