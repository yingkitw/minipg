...
use super::{DefaultIncrementalParser, IncrementalParser, SyntaxTree};
        ...
        for job in jobs {
            ...
            let result = match &job.source {
                Some(source) => match self.parser.parse(source) {
                    ...
                    Err(e) => {
                        ParseResult::failure(job.id.clone(), format!("{:?}", e), start.elapsed())
                    }
                },
...
mod tests {
    ...
    fn test_parse_result_success() {
        let grammar = minipg_antlr::ast::Grammar::new(
            "Test".to_string(),
    ...
    fn test_parallel_config_limits() {
        let config = ParallelConfig { max_workers: 4 };
