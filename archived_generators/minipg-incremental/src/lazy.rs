...
impl LazyParser {
    ...
    pub fn partition_source(&self, source: &str) -> Vec<ParsedRegion> {
        ...
        let start_line = visible_range
            .start
            .point
            .row
            .saturating_sub(self.config.buffer_lines);
        let end_line = (visible_range.end.point.row + self.config.buffer_lines).min(total_lines);

        // Create regions with priorities
        vec![ParsedRegion::new(
            Range::new(
                Position::new(
                    self.byte_offset_for_line(source, start_line),
                    Point::new(start_line, 0),
                ),
                Position::new(
                    self.byte_offset_for_line(source, end_line),
...
mod tests {
    ...
    fn test_partition_with_zero_buffer() {
        let config = LazyConfig { buffer_lines: 0 };
        let parser = LazyParser::new(config);
        let source = "line1\nline2\nline3\nline4\nline5";
    ...
    fn test_config_custom_buffer() {
        let config = LazyConfig { buffer_lines: 50 };
        let parser = LazyParser::new(config);
    ...
    fn test_visible_range_full_document() {
        ...
        let visible = Range::new(Position::zero(), parser.end_position(source));
        parser.set_visible_range(visible);
