...
use minipg_core::{types::CodeGenConfig, CodeGenerator, Result};
...
impl GoCodeGenerator {
    ...
    fn generate_package_header(&self, grammar: &Grammar) -> String {
        ...
        code.push_str(&format!(
            "// Package {} provides a parser generated by minipg.\n",
    ...
    fn generate_lexer(&self, grammar: &Grammar) -> String {
        ...
        code.push_str(&format!(
            "// {}Lexer tokenizes input for the {} grammar.\n",
        ...
        code.push_str(&format!(
            "// New{}Lexer creates a new lexer from the input string.\n",
            grammar.name
        ));
        code.push_str(&format!(
            "func New{}Lexer(input string) *{}Lexer {{\n",
            grammar.name, grammar.name
        ));
        ...
        code.push_str(
            "\t\t\tMessage:  fmt.Sprintf(\"Unexpected character: '%s'\", invalidChar),\n",
        );
        code.push_str("\t\t\tPosition: startPos,\n");
        code.push_str("\t\t}\n");
        code.push_str("\t}\n");
        ...
        let lexer_rules_for_helpers: Vec<_> =
            grammar.lexer_rules().filter(|r| !r.is_fragment).collect();
        ...
        code.push_str(&format!(
            "func (l *{}Lexer) TokenizeAll() ([]*Token, []*ParseError) {{\n",
            grammar.name
        ));
    ...
    fn generate_parser(&self, grammar: &Grammar) -> String {
        ...
        code.push_str(&format!(
            "// {}Parser parses input according to the {} grammar.\n",
            grammar.name, grammar.name
        ));
        ...
        code.push_str(&format!(
            "// New{}Parser creates a new parser from the input string.\n",
            grammar.name
        ));
        code.push_str(&format!(
            "func New{}Parser(input string) *{}Parser {{\n",
            grammar.name, grammar.name
        ));
    ...
    fn generate_parser_method(&self, rule: &Rule) -> String {
        ...
        if rule.returns.is_empty() {
            ...
            let return_type = rule.returns[0]
                .return_type
                .as_ref()
                .map(|t| t.as_str())
                .unwrap_or("interface{}");
            ...
            crate::ast::Element::Terminal {
                ...
                if *is_list {
                    if let Some(lbl) = label {
                        ...
                        code.push_str(&format!(
                            "\tfor p.currentToken != nil && p.currentToken.Kind == Token{} {{\n",
                            value
                        ));
                    ...
                    code.push_str(&format!(
                        "\tif p.currentToken == nil || p.currentToken.Kind != Token{} {{\n",
                        value
                    ));
                    code.push_str(&format!(
                        "\t\treturn fmt.Errorf(\"expected token {}, got %%v\", p.currentToken)\n",
                        value
                    ));
                    code.push_str(&format!("\t}}\n"));
                    if let Some(lbl) = label {
