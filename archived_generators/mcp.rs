...
use minipg_antlr::analysis::SemanticAnalyzer;
use minipg_antlr::codegen::{RustCodeGenerator, PythonCodeGenerator, JavaScriptCodeGenerator};
use minipg_core::{types::CodeGenConfig, GrammarParser, SemanticAnalyzer as SemanticAnalyzerTrait, CodeGenerator as CodeGeneratorTrait};
use minipg_antlr::parser::GrammarParser as Parser;
use rmcp::{
...
impl MinipgServer {
    ...
    pub async fn validate_grammar(
        ...
        let has_warnings = analysis.diagnostics.iter()
            .any(|d| matches!(d.severity, minipg_core::DiagnosticSeverity::Warning));
    ...
    pub async fn list_target_languages(
        ...
        let languages = vec![
            LanguageInfo { name: "rust".to_string(), description: "Rust language with optimized DFA".to_string() },
            LanguageInfo { name: "python".to_string(), description: "Python 3.10+ with type hints".to_string() },
