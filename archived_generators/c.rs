...
use minipg_core::types::CodeGenConfig;
use minipg_core::{CodeGenerator, Result};
...
impl CCodeGenerator {
    ...
    fn generate_header(&self, grammar: &Grammar) -> String {
        ...
        let mut token_types: Vec<String> = grammar.lexer_rules().map(|r| r.name.clone()).collect();
        token_types.sort();
        ...
        for rule in grammar.parser_rules() {
            code.push_str(&format!(
                "ParseError* parse_{}(Parser *parser);\n",
                rule.name
            ));
        }
    ...
    fn generate_source(&self, grammar: &Grammar) -> String {
        ...
        code.push_str(
            "        return make_token(TOKEN_EOF, \"\", lexer->line, lexer->column, 0);\n",
        );
        code.push_str("    }\n");
        ...
        let lexer_rules: Vec<_> = grammar.lexer_rules().filter(|r| !r.is_fragment).collect();
        ...
        if !lexer_rules.is_empty() {
            ...
            for (i, rule) in lexer_rules.iter().enumerate() {
                ...
                code.push_str(&format!(
                    "        return make_token(TOKEN_{}, text, lexer->line, lexer->column, len);\n",
        ...
        code.push_str(
            "        return make_token(TOKEN_ERROR, \"\", lexer->line, lexer->column, 0);\n",
        );
        code.push_str("    }\n\n");
            ...
            crate::ast::Element::RuleRef {
                ...
                if *is_list {
                    if let Some(lbl) = label {
                        ...
                        code.push_str(&format!(
                            "    void ***{} = safe_malloc({}_capacity * sizeof(void*));\n",
                            lbl, lbl
                        ));
                        code.push_str(&format!("    while (1) {{\n"));
                        code.push_str(&format!(
                            "        ParseError *err = parse_{}(parser);\n",
                            name
                        ));
                        ...
                        code.push_str(&format!(
                            "            {} = safe_realloc({}, {}_capacity * sizeof(void*));\n",
                            lbl, lbl, lbl
                        ));
                        code.push_str(&format!("        }}\n"));
                        code.push_str(&format!(
                            "        // Store result would go here (need AST node type)\n"
                        ));
                    ...
                    if let Some(lbl) = label {
                        ...
                        code.push_str(&format!(
                            "    // Note: Result type depends on rule return type\n"
                        ));
            ...
            crate::ast::Element::Terminal {
                ...
                if *is_list {
                    if let Some(lbl) = label {
                        ...
                        code.push_str(&format!("    size_t {}_capacity = 4;\n", lbl));
                        code.push_str(&format!(
                            "    Token *{} = safe_malloc({}_capacity * sizeof(Token));\n",
                            lbl, lbl
                        ));
                        code.push_str(&format!(
                            "    while (parser->current_token.type == TOKEN_{}) {{\n",
                            value.to_uppercase()
                        ));
                        ...
                        code.push_str(&format!("            {}_capacity *= 2;\n", lbl));
                        code.push_str(&format!(
                            "            {} = safe_realloc({}, {}_capacity * sizeof(Token));\n",
                            lbl, lbl, lbl
                        ));
                        code.push_str(&format!("        }}\n"));
                        code.push_str(&format!(
                            "        {}[{}_count++] = parser->current_token;\n",
                            lbl, lbl
                        ));
                    ...
                    code.push_str(&format!(
                        "    if (parser->current_token.type != TOKEN_{}) {{\n",
                        value.to_uppercase()
                    ));
                    ...
                    code.push_str(&format!(
                        "        err->message = safe_strdup(\"Expected token {}\");\n",
                        value
                    ));
...
mod tests {
    ...
    fn test_c_codegen_basic() {
        ...
        let grammar = parser
            .parse_string(grammar_text, "test.g4")
